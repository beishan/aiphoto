...
const dockPreviewStyle = computed(() => ({
  ...
  '--preview-size': `${Math.round(dock.value.iconSize * 1.5)}px`,
  '--preview-scale': String(dock.value.maxScale),
  '--preview-gap': `${Math.max(10, Math.round(dock.value.iconSize * 0.55))}px`,
  '--preview-spread': `${Math.round((dock.value.maxScale - 1) * dock.value.iconSize * 0.75)}px`,
}))
